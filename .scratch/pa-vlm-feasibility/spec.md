...
- The metadata-scrubbed photographs, frozen manifest, independent annotations, versioned prompts and schemas, evaluator command, and [execution report](../../docs/vlm-feasibility.md) now exist. The local `results/` directory holds the measured v1 development and v2 development/held-out trials. The held-out cohort has been consumed by one v2 scoring run; it cannot become a fresh held-out set after further tuning.
- The user's physical inspection remains the reference; model output must never repair missing ground truth. Unknown acceptable PA ranges remain unknown. Missing or disputed views and line anchors stay unresolved in the report.
