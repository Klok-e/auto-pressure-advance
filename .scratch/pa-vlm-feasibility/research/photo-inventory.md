...
The detail frames show readable herringbone geometry but embossed white-on-white labels, glare, and overlapping neighbouring patterns. The independently checked memberships and frozen split are recorded in `fixtures/manifest.json` and `fixtures/reference.json`; `20260924_191547` remains unassigned because its automatic cross-view match is uncertain. Do not infer identity from file order. The evaluator isolates one pattern before a model request and rejects a development crop that exposes a held-out region. The project copies have the same filenames under `fixtures/images/`; their hashes differ only because metadata was removed or replaced with orientation-only EXIF.
